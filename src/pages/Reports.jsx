...
import { DEPT_PERFORMANCE, EMPLOYEES, formatHours, getProductivityColor } from '../data/mockData';
    ...
    <div className="page-content animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 20 }}>
        <div className="chart-container">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={activeReport === 'annual' ? TREND_DATA : WEEKLY_DATA} barGap={4}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        <div className="chart-container">
          ...
          {DEPT_PERFORMANCE.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--text-muted)' }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>🏢</div>
              <div style={{ fontSize: '0.8125rem' }}>No data yet — available after Monday</div>
            </div>
          ) : (
      ...
      <div className="card" style={{ overflow: 'hidden', marginBottom: 20 }}>
        ...
        {topEmployees.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 24px', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: 4 }}>Leaderboard is empty</div>
            <div style={{ fontSize: '0.875rem' }}>Employee rankings will appear here after Monday's data is logged</div>
          </div>
        ) : (
          <table>
            <thead>
              <tr>
                <th>Rank</th><th>Employee</th><th>Dept</th>
                <th>ስራ ላይ የዋለ ሰዓት</th><th>የባከነ ሰዓት</th>
                <th>Overtime</th><th>Productivity</th><th>Score</th>
              </tr>
            </thead>
            <tbody>
              {topEmployees.map((emp, i) => (
                <tr key={emp.id}>
                  <td><span style={{ fontSize: '1.125rem' }}>{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i + 1}`}</span></td>
                  <td>
                  ...
                  <td style={{ color: emp.workedHours >= 8 ? '#10b981' : '#ef4444', fontWeight: 600 }}>{formatHours(Math.max(0, 8 - emp.workedHours))}h</td>
                  <td style={{ color: '#3b82f6', fontWeight: 600 }}>{formatHours(Math.max(0, emp.workedHours - 8))}h</td>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontWeight: 600, color: getProductivityColor(emp.productivity), fontSize: '0.875rem' }}>{emp.productivity}%</span>
                    </div>
                  </td>
                  <td>
