...
import { DEPARTMENTS, PROJECTS, calcTimeMetrics, formatHours, ATTENDANCE_RECORDS } from '../data/mockData';

function TimeMetricCard({ label, labelAm, value, icon, color, bg }) {
  return (
...
export default function Attendance() {
  ...
  const statusConfig = {
    ...
    'overtime': { label: 'Overtime', color: '#3b82f6', bg: 'rgba(59,130,246,0.1)', icon: '🔵' },
  ...
  return (
    <div className="page-content animate-fade-in">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Track your daily working hours and productivity</p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input type="number" name="breakMinutes" value={form.breakMinutes} onChange={handleChange} className="form-control" min={0} max={240} placeholder="60" />
              </div>
              ...
              <div className="form-group">
                ...
                <select name="project" value={form.project} onChange={handleChange} className="form-control">
          ...
          <div>
            ...
            {metrics ? (
              <>
                {/* Status banner */}
                {metrics.status && statusConfig[metrics.status] && (
                  <div style={{
                    padding: '14px 16px',
                ...
                <div className="grid grid-cols-2" style={{ gap: 12, marginBottom: 16 }}>
                  ...
                  <TimeMetricCard
                    ...
                    icon="📈"
                    color={metrics.productivity >= 100 ? '#3b82f6' : metrics.productivity >= 90 ? '#10b981' : metrics.productivity >= 75 ? '#f59e0b' : '#ef4444'}
                  />
                </div>
              ...
              <div className="card" style={{ padding: '2rem', textAlign: 'center' }}>
                ...
                <div style={{ marginTop: 16, padding: '12px', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
                  <div style={{ fontFamily: "'Noto Sans Ethiopic', sans-serif", color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
                    ✅ ስራ ላይ የዋለ ሰዓት: —<br />
                    ⚠️ የባከነ ሰዓት: —<br />
                    📈 Productivity: —
                  </div>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          <div className="table-wrapper" style={{ border: 'none', borderRadius: 0 }}>
            <table>
              ...
              <tbody>
                {ATTENDANCE_RECORDS.slice(0, 20).map(rec => (
                  <tr key={rec.id}>
                    ...
                    <td style={{ fontWeight: 600, color: rec.workedHours >= 8 ? '#10b981' : '#ef4444' }}>
                      {formatHours(Math.max(0, 8 - rec.workedHours))}h
                    </td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{
                          ...
                          <div style={{
                            width: `${Math.min(rec.productivity, 100)}%`, height: '100%',
                    ...
                    <td>
                      <span className={`badge ${rec.status === 'complete' || rec.status === 'overtime' ? 'badge-success' : 'badge-warning'}`}>
      ...
      {activeTab === 'timesheet' && (
        <div className="card" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h3 style={{ fontWeight: 700 }}>Monthly Timesheet — {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h3>
            <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div className="grid grid-cols-4" style={{ gap: 16, marginBottom: 20 }}>
            {[
              { label: 'Total Days', value: 22, icon: '📅', color: '#2563EB' },
              { label: 'Present', value: 20, icon: '✅', color: '#10b981' },
              { label: 'Total Hours', value: '162h', icon: '⏱️', color: '#7c3aed' },
              { label: 'Overtime', value: '14h', icon: '🔵', color: '#3b82f6' },
            ].map((item, i) => (
          ...
          <div className="table-wrapper">
            <table>
              ...
              <tbody>
                {[1, 2, 3, 4].map(week => {
                  const days = Array.from({ length: 5 }, () => (6 + Math.random() * 4).toFixed(1));
                  const total = days.reduce((s, d) => s + parseFloat(d), 0);
                  return (
                    <tr key={week}>
                      <td style={{ fontWeight: 600 }}>Week {week}</td>
                      {days.map((d, i) => (
                        <td key={i} style={{ color: parseFloat(d) >= 8 ? '#10b981' : '#f59e0b', fontWeight: 500 }}>
                          {d}h
                        </td>
                      ))}
                      <td style={{ fontWeight: 700 }}>{total.toFixed(1)}h</td>
                      <td>
                        <span className={`badge ${total >= 40 ? 'badge-success' : 'badge-warning'}`}>
                          {total >= 40 ? 'Complete' : 'Incomplete'}
                        </span>
                      </td>
                    </tr>
                  );
